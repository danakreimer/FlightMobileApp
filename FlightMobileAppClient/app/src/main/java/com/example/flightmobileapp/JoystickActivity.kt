...
class JoystickView @JvmOverloads constructor(
    ...
    */
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val d: Int = min(measure(widthMeasureSpec), measure(heightMeasureSpec))
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        if (abs > (mBorderRadius - mButtonRadius) || (isButtonStickToBorder && abs != 0.0)) {
            mPosX = ((mPosX - mCenterX) * (mBorderRadius - mButtonRadius) / abs + mCenterX).toInt()
            mPosY = ((mPosY - mCenterY) * (mBorderRadius - mButtonRadius) / abs + mCenterY).toInt()
        }
        if (!isAutoReCenterButton) {
